...
const CheckoutScreen = ({navigation}) => {
  ...
  const [selectedPaymentMethod, setSelectedPaymentMethod] = useState(null); // nhận trạng thái thanh toán
              ...
              <DualTextRow
                ...
                leftText={
                  cartState.deliveryMethod === DeliveryMethod.PICK_UP.value
                    ? DeliveryMethod.PICK_UP.label.toLocaleUpperCase()
                    : DeliveryMethod.DELIVERY.label.toLocaleUpperCase()
                }
                rightText={'Thay đổi'}
      ...
      <ActionDialog
        ...
        onApprove={async () => {
          try {
            ...
            // Điều hướng dựa vào phương thức thanh toán đã chọn
            if (selectedPaymentMethod?.value === 'cash') {
              Alert.alert('Thông báo', 'Đặt hàng thành công!');
              navigation.goBack(); // Quay lại màn hình trước đó
            } else if (selectedPaymentMethod?.value === 'PayOs') {
              if (response?.data?._id && response?.data?.totalPrice) {
    ...
    <Column style={styles.containerItem}>
      ...
      {timeInfo && timeInfo.fulfillmentDateTime ? (
        <>
          ...
          <NormalText
            text={`${timeInfo.selectedDay} - ${timeInfo.selectedTime}`}
          />
        </>
      ) : (
        <>
...
const AddressSection = ({cartState, chooseMerchant, chooseUserAddress}) => {
  return (
    <View style={styles.containerItem}>
      ...
      />
      {cartState.deliveryMethod === DeliveryMethod.PICK_UP.value ? (
        cartState?.storeInfo?.storeName &&
        cartState?.storeInfo?.storeAddress ? (
          <>
            <TitleText
              text={cartState?.storeInfo?.storeName}
        ...
        <NormalText
          text={`${cartState?.shippingAddressInfo?.location}`}
          style={{lineHeight: 20}}
        />
      ) : (
        <NormalText
...
const PaymentDetailsView = ({
  ...
  return (
    <View
      ...
      <PaymentMethodView onSelectPaymentMethod={onSelectPaymentMethod} />
    </View>
  );
};

const PaymentMethodView = ({onSelectPaymentMethod}) => {
