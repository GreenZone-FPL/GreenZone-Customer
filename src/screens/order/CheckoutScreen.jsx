...
const CheckoutScreen = props => {
  ...
  const navigationInformationSheet = () => {
    navigation.navigate(ScreenEnum.ChangeRecipientInformationSheet);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.containerContent}>
        ...
        <RecipientInfo
          navigationInformationSheet={navigationInformationSheet}
        />
        <FlatList
          data={products}
...
const RecipientInfo = ({navigationInformationSheet}) => {
  return (
    <>
      <DualTextRow
        ...
        rightTextStyle={{color: colors.primary}}
        onRightPress={() => navigationInformationSheet()}
      />
...
const AddressSection = () => (
  <>
    ...
    <Text style={styles.normalText}>
      FPT Polytechnic TP. HCM - Tòa F, Công Viên Phần Mềm Quang Trung, Tòa nhà
      GenPacific Lô 3 đường 16, Trung Mỹ Tây, Quận 12, Hồ Chí Minh
    </Text>
  </>
);
    ...
    <View style={styles.row}>
      <Pressable style={styles.row} onPress={onChange}>
        <View style={[styles.circleWrapper, {backgroundColor: badgeBgColor}]}>
          <Text style={[styles.normalText, {color: badgeTextColor}]}>
            {quantityOfVoucher}
          </Text>
        </View>

        <Text style={[styles.greenText, {color: textColor, marginRight: 8}]}>
          Ưu đãi
        </Text>
        <AntDesign name="down" color={colors.primary} size={14} />
      </Pressable>

      <Text style={[styles.greenText, {color: textColor}]}>
        Tiết kiệm {money}đ
      </Text>
    </View>
  );
